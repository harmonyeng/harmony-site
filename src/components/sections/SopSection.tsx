...
export function SopSection({ sop }: SopSectionProps) {
  return (
    <section>
      {/* Clickable header bar → full SOP archive */}
      ...
      {sop ? (
        <>
          ...
          <p className="font-serif text-[15.5px] leading-[1.88] text-muted font-light italic mb-7">
            {sop.excerpt}
          ...
          >
            Read Full SOP <span className="inline-block transition-transform group-hover:translate-x-1">→</span>
          </Link>
        </>
      ) : (
